...
class FoodImageAnalyzer:
    ...
    def __init__(self, model_name: str = "gpt-4o", temperature: float = 0.1, max_tokens: int = 1000):
        ...
        - Выбирай наиболее подходящую единицу измерения:
          * "штук" - для целых предметов (яйца, яблоки, печенья, котлеты, булочки)
          * "грамм" - для блюд без четких границ (каши, салаты, жидкости, пюре)
          * "чашка" - для напитков и жидких блюд в чашках/стаканах
          * "кусок" - для продуктов, которые режутся кусками (торт, сыр, мясо)
          * "ломтик" - для хлеба, колбасы, нарезанных овощей
