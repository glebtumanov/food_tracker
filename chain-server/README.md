...
Для долгих операций доступна очередь задач. Это позволяет быстро отвечать клиенту и обрабатывать анализ в фоне.

- Статусы: `queued` → `processing` → `done` | `error`
- Хранилище: SQLite файл `jobs.sqlite3` в папке `chain-server`

Примеры запросов:

1) Создать задачу (любой из источников изображения — локальный путь, base64 или URL):
...
```json
{
  ...
  "updated_at": "2025-09-20T10:10:50Z",
  "error": null,
  "result": { "analysis": { ... }, "nutrients": { ... } },
  "image_url": "https://example.com/food.jpg"
}
```
...
- Синхронные эндпоинты `/api/v1/analyze*` сохраняются как есть, но используют ту же общую функцию анализа, что и job‑воркер.
- Временные файлы (base64/URL) сохраняются в `temp_images/` и удаляются после обработки.
- Для продакшена рекомендуется вынести SQLite в отдельный том и настроить бэкапы/ротацию.
...
├── jobs.sqlite3         # База задач (SQLite) — создаётся автоматически
...
- Асинхронная обработка запросов (реализовано через очередь Job)
